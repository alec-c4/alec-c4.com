...
Note the Postgres credentials need to be added to two places - the app's own `env.secret` (so `database.yml` can read them inside the app container) and `accessories.postgres.env.secret` (so the Postgres container can bootstrap itself in `db/init.sql`). They're two separate containers with two separate environments.
...
+POSTGRES_HOST=$(dotenvx get POSTGRES_HOST --quiet -f .env)
```
...
Note the `--chown` flag on `COPY` - without it, files are always copied as root regardless of any preceding `USER` instruction, so relying on `USER` alone here would leave `db`, `log`, `storage`, and `tmp` root-owned while the app runs as uid 1000. Using `--chown` at copy time also skips a separate recursive `chown -R` layer, which is slow once your `vendor/bundle` tree grows.
...
Note the Postgres credentials appear twice - once under the app's own `env.secret` so `database.yml` can read them inside the app container, and once under `accessories.postgres.env.secret` so the Postgres container can bootstrap itself. These are two separate containers with two separate environments; adding a secret to one does not expose it to the other.
...
POSTGRES_PORT='5432'
POSTGRES_HOST='super_duper_app_postgres'
```
